...
const EditableSubevent = () => {
    ...
    const { register, handleSubmit, setValue, getValues, setError, control, reset, watch, formState: { errors } } = useForm({
        defaultValues: {
            mode: "onSubmit",
    ...
    const fetchSubEvent = async () => {
        try {
            ...
            reset({
                ...
                MaxVoleenters: subEvent.maxVoleenters,
                isPaid: subEvent.price > 0,
                PaymentMode: subEvent.QrScanner !== null,
                Qrimg: []
            });
            if (subEvent.QrScanner !== null) {
                setPreview(subEvent.QrScanner)
            }
        } catch (error) {
            console.log(error)
        }
    ...
    const onsubmit = async (data) => {
        ...
        const file = data.Qrimg && data.Qrimg[0];
                                        ...
                                        <div className="mt-6">
                                            ...
                                            <div className="flex items-center space-x-3">
                                                <div
                                                    ...
                                                    className={`relative w-14 h-8 rounded-full transition-colors duration-300 cursor-pointer ${isPaid ? 'bg-green-500/60' : 'bg-blue-500/60'}`}
                                                >
                                                    <div
                                                        className={`absolute top-1 left-1 w-6 h-6 bg-white rounded-full shadow-md transform transition-transform duration-300 ${isPaid ? 'translate-x-6' : 'translate-x-0'}`}
                                                    />
                                                </div>
                                                <span className={`text-sm font-medium ${isPaid ? 'text-green-400' : 'text-blue-400'}`}>
                                                    {isPaid ? "Paid" : "Free"}
                                                </span>
                                            </div>
                                        </div>

                                        {isPaid && <div className="mt-6">
