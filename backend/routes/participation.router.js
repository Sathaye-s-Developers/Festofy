  ...
  try {
    ...
    if (alreadyRegistered) {
      return res.status(409).json({
        success: false,
        error: "You are already registered for this event/sub-event",
      });
    }
    ...
    if (subEventId) {
      ...
      if (!subEvent)
        return res
          .status(404)
          .json({ success: false, error: "Sub-event not found" });
    }
    ...
    if (maxAllowed && currentCount >= maxAllowed) {
      return res
        .status(400)
        .json({ success: false, error: "Participant limit reached" });
    }
    ...
    if (subEvent?.participation_type === "team") {
      if (!teamName || !members?.length) {
        return res.status(400).json({
          success: false,
          error: "Team name and members required for team event",
        });
      }
